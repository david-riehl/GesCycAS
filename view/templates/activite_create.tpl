				...
				<form name="frm_droit_create" action="" method="post">
					<div  id="main_items">
						<table>
							...
							<tr>
								<th><label for="place">place disponible{if isset($item_fields[2].constraint) && $item_fields[2].constraint=="NOT NULL"}<span class="required">*</span>{/if} : </label></th>
								<td><input type="text" id="place" name="place" value="" maxlength="{$item_fields[2].size}"{if isset($item_fields[2].constraint) && $item_fields[2].constraint=="NOT NULL"} required="required"{/if}></td>
							</tr>
							<tr>
