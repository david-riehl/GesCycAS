{include file="document_header.tpl"}
			<div id="menu_left">
				<ul id="buttons">
{if in_array('Seance::create',$droits)}
...
{if in_array('Seance::create',$droits)}
				...
				<button type="submit" name="btn_create" class="create" title="Ajouter"></button>Ajouter une seance
				</form>
{/if}
{else}
				<table class="seance_list">
					<tr>
						<th>date</th>
{if in_array('Seance::create',$droits)}
...
{section name=list loop=$item}
					<tr class="{if $smarty.section.list.index %2 == 0}pair{else}impair{/if}">
						<td style="width: {$fields[1].size*10}px;">{$item[list].date}</td>
{if in_array('Seance::read',$droits)}
						<td>
...
{/if}
{if in_array('Seance::update',$droits)}
						<td>
...
{/if}
{if in_array('Seance::delete',$droits)}
						<td>
